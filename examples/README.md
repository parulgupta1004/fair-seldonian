...
Runnable examples for the `fair-seldonian` library. Each script is
self-contained and prints its results; run any of them with:
...
| [`real_world_adult.py`](real_world_adult.py) | A real dataset (UCI Adult income): an unconstrained model's true-positive-rate gap vs. QSA refusing to certify it. Needs network access on first run. |
...
- The synthetic generator (`get_data`) produces a trivially separable feature,
  ...
  machinery, not model difficulty. `real_world_adult.py` shows realistic numbers.
