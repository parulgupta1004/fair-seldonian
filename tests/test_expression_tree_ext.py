...
from fair_seldonian.constraints.expression_tree_ext import (
    construct_expr_tree, eval_expr_tree, eval_expr_tree_conf_interval,
    isConstant, check_node_dup,
)
...
def test_bound_unifies_deltas():
    ...
    def collect(n):
        if n:
            collect(n.left)
            if n.value == "TP(1)": deltas.append(n.delta)
            collect(n.right)
    collect(t)
...
def test_optimizations_tighten():
    ...
    _, hi_base = eval_expr_tree_conf_interval(
        ...
        Y, pred, T, Inequality.HOEFFDING_INEQUALITY, 1, True, False)
    _, hi_opt = eval_expr_tree_conf_interval(
        construct_expr_tree(expr, 0.05, True, True),
        Y, pred, T, Inequality.HOEFFDING_INEQUALITY, 1, True, True)
