...
from fair_seldonian.constraints.expression_tree import (
    construct_expr_tree_base, eval_expr_tree_base,
    eval_expr_tree_conf_interval_base, isOperator, isMod, isFunc,
)
...
def test_eval():
    Y, pred, T = _data()
    assert float(eval_expr_tree_base(construct_expr_tree_base("TP(1) TP(0) -"), Y, pred, T)) is not None

def test_eval_abs():
    Y, pred, T = _data()
    assert float(eval_expr_tree_base(construct_expr_tree_base("TP(1) TP(0) - abs"), Y, pred, T)) >= 0
...
def test_hoeffding_interval():
    ...
    lo, hi = eval_expr_tree_conf_interval_base(
        ...
        Y, pred, T, 0.05, Inequality.HOEFFDING_INEQUALITY, 1, True, False)
...
def test_ttest_interval():
    ...
    lo, hi = eval_expr_tree_conf_interval_base(
        construct_expr_tree_base("TP(1) TP(0) -"),
        Y, pred, T, 0.05, Inequality.T_TEST, 1, True, False)
    assert float(lo) <= float(hi)

def test_smaller_delta_widens():
    Y, pred, T = _data()
    tree = construct_expr_tree_base("TP(1)")
    _, hi_strict = eval_expr_tree_conf_interval_base(tree, Y, pred, T, 0.01, Inequality.HOEFFDING_INEQUALITY, 1, True, False)
    _, hi_loose = eval_expr_tree_conf_interval_base(tree, Y, pred, T, 0.10, Inequality.HOEFFDING_INEQUALITY, 1, True, False)
