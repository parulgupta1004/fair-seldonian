import numpy as np
import torch
from fair_seldonian.models.logistic_regression import predict, fHat, simple_logistic, eval_ghat, ghat
...
def test_predict_range():
    p = predict(torch.randn(5, dtype=torch.float64), torch.tensor([0.0], dtype=torch.float64), np.random.randn(20, 5))
...
def test_predict_zero_weights():
    p = predict(torch.zeros(5, dtype=torch.float64), torch.tensor([0.0], dtype=torch.float64), np.random.randn(10, 5))
    assert torch.allclose(p, torch.full_like(p, 0.5))

def test_fhat():
    loss = fHat(torch.randn(5, dtype=torch.float64), torch.tensor([0.0], dtype=torch.float64),
                np.random.randn(30, 5), np.random.randint(0, 2, 30).astype(float))
